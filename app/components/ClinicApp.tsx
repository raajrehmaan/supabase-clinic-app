...
export default function ClinicApp() {
  ...
  async function loadData() {
    try {
      ...
      setData({
        users: (users.data || []).map(fromUserRow),
  ...
  async function login(event: React.FormEvent) {
    ...
    const user = data.users.find((item) => item.username === cleanUsername(username) && item.passwordHash === hash);
      ...
      <main className="login">
        <form className="card" onSubmit={authMode === "login" ? login : createAccount}>
          ...
          <div className="form">
            ...
            {authMode === "create" ? <label className="full">Admin creation code<input value={adminCode} onChange={(event) => setAdminCode(event.target.value)} type="password" placeholder="Enter 6871 only for Admin" /></label> : null}
